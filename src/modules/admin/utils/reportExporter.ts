...
export function exportReportPDFBrowser(
  ...
  doc.setFontSize(18);
  doc.text(cab.titulo.toUpperCase(), pageWidth / 2, yPosition, {
    align: "center",
  });
  ...
  if (rows.length) {
    ...
    // Header
    cols.forEach((c, i) => {
      doc.text(etiquetaCampo[c] || c, 20 + i * colWidth, yPosition);
    });
    ...
    rows.forEach((row) => {
      ...
      cols.forEach((col, colIndex) => {
        ...
        doc.text(formatearValor(value), 20 + colIndex * colWidth, yPosition);
      });
      yPosition += 7;
    });
  }
...
export function exportOpportunityNumbersPDFBrowser(
  data: OpportunityNumbersReport
) {
...
export function exportOpportunityStatsPDFBrowser(
  data: StatusOpportunitiesReport
) {
  return exportReportPDFBrowser(
...
export function exportOpportunityNumbersExcelBrowser(
  data: OpportunityNumbersReport
) {
...
export function exportOpportunityStatsExcelBrowser(
  data: StatusOpportunitiesReport
) {
  return exportReportExcelBrowser(
    prepareOpportunityStatsData({ ...data, type: "ESTADO OPORTUNIDADES" }),
...
function prepareOpportunityStatsData(
  report: StatusOpportunitiesReport
): OpportunityNumbersReport {
  return {
    ...report,
