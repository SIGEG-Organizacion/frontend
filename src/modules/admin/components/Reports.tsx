...
const Reports: React.FC = () => {
  ...
  const [selectedReport, setSelectedReport] = useState<'numbers' | 'status' | 'interest' | 'users'>('numbers')
  ...
  const handleGenerateReport = async (
    ...
    try {
      ...
      if (type === 'users') {
        ...
        if (filters.validated !== undefined) usedFilters.validated = filters.validated
      }
  ...
  return (
    <div className="space-y-6">
      <div className="border-b border-gray-200 pb-4">
        <h2 className="text-2xl font-semibold">{t('admin.reports.title', 'Reportes')}</h2>
        <p className="text-gray-500 mt-1">{t('admin.reports.description', 'Descarga reportes filtrados por fecha, empresa, estado, etc.')}</p>
      </div>
      ...
      <div className="bg-white rounded-lg shadow p-4 mb-6">
        <form className="flex flex-wrap gap-4 items-end">
          ...
          <div className="flex flex-col">
            <label className="text-xs font-medium text-gray-700 mb-1">{t('admin.reports.startDate', 'Fecha inicial')}</label>
            <input
              type="date"
          ...
          {/* Fecha final */}
          <div className="flex flex-col">
            <label className="text-xs font-medium text-gray-700 mb-1">{t('admin.reports.endDate', 'Fecha final')}</label>
            <input
              type="date"
          ...
          {/* Formato */}
          <div className="flex flex-col">
            <label className="text-xs font-medium text-gray-700 mb-1">{t('admin.reports.format', 'Formato')}</label>
            <select
              value={format}
          ...
          {/* Empresa */}
          <div className="flex flex-col">
            <label className="text-xs font-medium text-gray-700 mb-1">{t('company.list.filterCompanyLabel', 'Empresa')}</label>
            <input
              type="text"
              value={filters.companyName}
              onChange={e => setFilters(f => ({ ...f, companyName: e.target.value }))}
              className="rounded-md border-gray-300 shadow-sm px-2 py-1"
              placeholder={t('company.list.filterCompanyPlaceholder', 'Empresa...')}
            />
          </div>
          ...
          <div className="flex items-center h-8">
            <input
              ...
              onChange={e => setFilters(f => ({ ...f, forStudents: e.target.checked }))
              }
              className="mr-2"
              id="forStudents"
            />
            <label htmlFor="forStudents" className="text-xs font-medium text-gray-700">{t('opportunities.filters.forStudentsLabel', 'Solo para estudiantes')}</label>
          </div>
          {/* Agrupar por */}
          <div className="flex flex-col">
            <label className="text-xs font-medium text-gray-700 mb-1">{t('admin.reports.groupBy', 'Agrupar por')}</label>
            <select
              value={filters.groupBy}
              onChange={e => setFilters(f => ({ ...f, groupBy: e.target.value as 'day' | 'month' }))
              }
              className="rounded-md border-gray-300 shadow-sm px-2 py-1"
            >
              <option value="day">{t('admin.reports.groupByDay', 'Día')}</option>
              <option value="month">{t('admin.reports.groupByMonth', 'Mes')}</option>
            </select>
          </div>
          {/* Estado (solo para status e interest) */}
          {(selectedReport === 'status' || selectedReport === 'interest') && (
            <div className="flex flex-col">
              <label className="text-xs font-medium text-gray-700 mb-1">{t('company.list.filterStatusLabel', 'Estado')}</label>
              <select
              ...
              >
                ...
                <option value="open">{t('company.list.status.open', 'Abierta')}</option>
                <option value="closed">{t('company.list.status.closed', 'Cerrada')}</option>
                <option value="pending-approval">{t('company.list.status.pending-approval', 'Pendiente')}</option>
              </select>
            </div>
          )}
          ...
          {selectedReport === 'interest' && (
            <div className="flex flex-col">
              <label className="text-xs font-medium text-gray-700 mb-1">UUID</label>
              <input
                type="text"
                value={filters.uuid}
                onChange={e => setFilters(f => ({ ...f, uuid: e.target.value }))}
                className="rounded-md border-gray-300 shadow-sm px-2 py-1"
                placeholder="UUID..."
              />
            </div>
          ...
          {selectedReport === 'users' && (
            <>
              <div className="flex flex-col">
                <label className="text-xs font-medium text-gray-700 mb-1">{t('userMgmt.table.role', 'Rol')}</label>
                <select
                  value={filters.role || ''}
                  onChange={e => setFilters(f => ({ ...f, role: e.target.value as ReportFilters['role'] }))}
                  className="rounded-md border-gray-300 shadow-sm px-2 py-1"
                >
                  <option value="">{t('userMgmt.filterAll', 'Todos')}</option>
                  <option value="student">{t('userMgmt.role.student', 'Estudiante')}</option>
                  <option value="company">{t('userMgmt.role.company', 'Empresa')}</option>
                  <option value="adminLink">{t('userMgmt.role.adminLink', 'Admin Link')}</option>
                  <option value="vadminTFG">{t('userMgmt.role.adminTGF', 'Admin TFG')}</option>
                </select>
              </div>
              <div className="flex items-center h-8">
                <input
                  ...
                  onChange={e => setFilters(f => ({ ...f, validated: e.target.checked }))}
                  className="mr-2"
                  id="validated"
                />
                <label htmlFor="validated" className="text-xs font-medium text-gray-700">{t('userMgmt.table.validated', 'Solo validados')}</label>
              </div>
            </>
          )}
        </form>
