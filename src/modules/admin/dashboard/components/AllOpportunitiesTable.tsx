...
const AllOpportunitiesTable: React.FC<Props> = ({
  ...
  if (loading) {
    return (
      <div className="p-4 text-center">{t("admin.dashboard.loadingOpps")}</div>
    );
  }
        ...
        <thead className="bg-gray-100 sticky top-0">
          <tr>
            <th className="px-4 py-2 text-left">
              {t("admin.dashboard.table.colDescription")}
            </th>
            <th className="px-4 py-2 text-left">
              {t("admin.dashboard.table.colCompany")}
            </th>
            <th className="px-4 py-2 text-left">
              {t("admin.dashboard.table.colDeadline")}
            </th>
            <th className="px-4 py-2 text-left">
              {t("admin.dashboard.table.colMode")}
            </th>
            <th className="px-4 py-2 text-left">
              {t("admin.dashboard.table.colStatus")}
            </th>
            <th className="px-4 py-2 text-center">
              {t("admin.dashboard.table.colActions")}
            </th>
          </tr>
        </thead>
        <tbody className="divide-y divide-gray-200">
          {opportunities.map((op) => (
            <tr key={op.uuid} className="hover:bg-gray-50">
              ...
              <td className="px-4 py-2">
                {t(`opportunities.detail.modeList.${op.mode}`)}
              </td>
              <td className="px-4 py-2">
                {t(`opportunities.detail.statusList.${op.status}`)}
              </td>
